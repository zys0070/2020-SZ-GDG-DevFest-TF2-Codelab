...
# bliu29: copied from _find_ssd_head_node and made mirror changes
def _find_ssd_head_node_tf2(graph: Graph, ssd_head_index: int, head_type: str):
    """
    Finds the SSD head node with index 'ssd_head_index' in the topology. The parameter 'head_type' specifies what type
    of the head is requested: with box predictions or class predictions.
    :param graph: graph with the topology.
    ...
    :return: the requested Node or None if node is not found.
    """
    # bliu29: update head information according to TF2 graph structure
    if head_type == 'box':
        possible_node_names = ['StatefulPartitionedCall/BoxPredictor/ConvolutionalBoxHead_%d/BoxEncodingPredictor/BiasAdd' % ssd_head_index,
                               # TODO for FPN networks
                               'WeightSharedConvolutionalBoxPredictor/BoxPredictor/BiasAdd' if ssd_head_index == 0 else
                               'WeightSharedConvolutionalBoxPredictor_%d/BoxPredictor/BiasAdd' % ssd_head_index]
    elif head_type == 'class':
        possible_node_names = ['StatefulPartitionedCall/BoxPredictor/ConvolutionalClassHead_%d/ClassPredictor/BiasAdd' % ssd_head_index,
                               # TODO for FPN networks
                               'WeightSharedConvolutionalBoxPredictor/ClassPredictor/BiasAdd' if ssd_head_index == 0 else
                               'WeightSharedConvolutionalBoxPredictor_%d/ClassPredictor/BiasAdd' % ssd_head_index]
    else:
        raise Error('SSD heads can be of type "box" and "class" only.')
...
# bliu29: copied from _relax_reshape_nodes and made mirror changes
def _relax_reshape_nodes_tf2(graph: Graph, pipeline_config: PipelineConfig):
    """
    Finds the 'Reshape' operations following the SSD head nodes which have hard-coded output dimensions and replaces
    them with new ones with one of the dimensions sizes equal to -1. This function is used to make TF OD API SSD models
    reshape-able.
    ...
    if num_layers is None:
        num_layers = pipeline_config.get_param('multiscale_anchor_generator_max_level') - \
                     pipeline_config.get_param('multiscale_anchor_generator_min_level') + 1
    for ssd_head_ind in range(num_layers):
        # bliu29: replace _find_ssd_head_node with _find_ssd_head_node_tf2
        input_node = _find_ssd_head_node_tf2(graph, ssd_head_ind, 'box')
        assert (input_node is not None)
        old_reshape_node = _skip_node_of_type(input_node.out_node(),
                                              ['Identity', 'FakeQuantWithMinMaxVars', 'FakeQuantize'])
        assert old_reshape_node.op == 'Reshape'
        ...
        # fix hard-coded value for the number of items in tensor produced by the convolution to make topology reshapable
        # bliu29: replace _find_ssd_head_node with _find_ssd_head_node_tf2
        input_node = _find_ssd_head_node_tf2(graph, ssd_head_ind, 'class')
        assert (input_node is not None)
        old_reshape_node = _skip_node_of_type(input_node.out_node(),
                                              ['Identity', 'FakeQuantWithMinMaxVars', 'FakeQuantize'])
...
# bliu29: copied from _create_prior_boxes_node and made mirror changes
def _create_prior_boxes_node_tf2(graph: Graph, pipeline_config: PipelineConfig):
    """
    The function creates one or several PriorBoxClustered nodes based on information from the pipeline configuration
    files. The PriorBoxClustered nodes get input data from SSD 'heads' and from the placeholder node (just to get
    input image size).
    ...
    for ssd_head_ind in range(num_layers):
        # bliu29: replace _find_ssd_head_node with _find_ssd_head_node_tf2
        ssd_head_node = _find_ssd_head_node_tf2(graph, ssd_head_ind, 'box')
        assert (ssd_head_node is not None)
        ...
        # Cast operations is executed in the middle phase and it will fail when there are several consumers of the
        # Placeholder
        # bliu29: replace image_tensor with input_tensor
        prior_box_node = prior_box_op.create_node([ssd_head_node, Node(graph, 'input_tensor').out_node(0)],
                                                  {'name': 'PriorBoxClustered_{}'.format(ssd_head_ind)})
        prior_box_nodes.append(prior_box_node)
    if len(prior_box_nodes) == 1:
        return prior_box_nodes[0]
...
# bliu29: copied from ObjectDetectionAPIPreprocessorReplacement and made mirror changes
class ObjectDetectionAPIPreprocessorTF2Replacement(FrontReplacementFromConfigFileSubGraph):
    """
    The class replaces the "Preprocessor" block resizing input image and applying mean/scale values. Only nodes related
    to applying mean/scaling values are kept.
    """
    # bliu29: replace ObjectDetectionAPIPreprocessorReplacement with ObjectDetectionAPIPreprocessorTF2Replacement
    replacement_id = 'ObjectDetectionAPIPreprocessorTF2Replacement'
    ...
    def nodes_to_remove(self, graph: Graph, match: SubgraphMatch):
        new_nodes_to_remove = match.matched_nodes_names()
        # do not remove nodes that perform input image scaling and mean value subtraction
        # bliu29: update nodes' name according to TF2 graph structure
        for node_to_keep in ('StatefulPartitionedCall/Preprocessor/sub',
    ...
    # bliu29: made much changes specifically for TF2 OD preprocessing sub-graph structure
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        argv = graph.graph['cmd_params']
        layout = graph.graph['layout']
        if argv.tensorflow_object_detection_api_pipeline_config is None:
        ...
        # the output node of the new sub graph
        sub_node = match.output_node(0)[0]
        if not sub_node.has('op') or sub_node.op != 'Sub':
            raise Error('The output op of the Preprocessor sub-graph is not of type "Sub". Looks like the topology is '
                        'not created with TensorFlow Object Detection API.')

        initial_input_node_name = 'input_tensor'
        if initial_input_node_name not in graph.nodes():
            raise Error('Input node "{}" of the graph is not found. Do not run the Model Optimizer with '
                        '"--input" command line parameter.'.format(initial_input_node_name))
        placeholder_node = Node(graph, initial_input_node_name)
        ...
        # the output node of the old sub graph
        stack_node = Node(graph, 'StatefulPartitionedCall/Preprocessor/stack')
        stack_out_node = stack_node.out_node()
        #print(stack_out_node.in_edges())
        # connect sub node to stack_out_node directly
        graph.create_edge(sub_node, stack_out_node, 0, 0)
...
# bliu29: copied from ObjectDetectionAPISSDPostprocessorReplacement and made mirror changes
class ObjectDetectionAPISSDPostprocessorTF2Replacement(FrontReplacementFromConfigFileSubGraph):
    # bliu29: replace ObjectDetectionAPISSDPostprocessorReplacement with ObjectDetectionAPISSDPostprocessorTF2Replacement
    replacement_id = 'ObjectDetectionAPISSDPostprocessorTF2Replacement'

    def run_after(self):
        # bliu29: replace ObjectDetectionAPIPreprocessorReplacement with ObjectDetectionAPIPreprocessorTF2Replacement
        return [ObjectDetectionAPIPreprocessorTF2Replacement, FakeQuantWithMinMaxVarsToQuantize]
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        if ('disable_prior_boxes_layers_generator' not in custom_attributes or
            ...
            # change the Reshape operations with hardcoded number of output elements of the convolution nodes to be
            # reshapable
            # bliu29: replace _relax_reshape_nodes with _relax_reshape_nodes_tf2
            _relax_reshape_nodes_tf2(graph, pipeline_config)

            # create PriorBoxClustered nodes instead of a constant value with prior boxes so the model could be reshaped
            if pipeline_config.get_param('ssd_anchor_generator_num_layers') is not None:
                # bliu29: replace _create_prior_boxes_node with _create_prior_boxes_node_tf2
                priors_node = _create_prior_boxes_node_tf2(graph, pipeline_config)
            elif pipeline_config.get_param('multiscale_anchor_generator_min_level') is not None:
                priors_node = _create_multiscale_prior_boxes_node(graph, pipeline_config)
        else:
            log.info('The anchor generator is not known. Save constant with prior-boxes to IR.')
